...
names(outcome.dat)
data <- data.1 %>%
  left_join(outcome.dat, by = c("CountyFIPS"))
...
choropleth <- ggplot() +
  geom_sf(data = merged_sf, aes(fill = STROKE_AdjPrev), color = "white", size = 0.1) +
...
# 8. Global Moran's I
moran_global <- moran.test(merged_proj$STROKE_AdjPrev, lw, zero.policy = TRUE)
print(moran_global)

moran_global_CA <- moran.test(merged_proj_CA$STROKE_AdjPrev, lw_CA, zero.policy = TRUE)
print(moran_global_CA)
moran_global_GA <- moran.test(merged_proj_GA$STROKE_AdjPrev, lw_GA, zero.policy = TRUE)
print(moran_global_GA)
moran_global_TX <- moran.test(merged_proj_TX$STROKE_AdjPrev, lw_TX, zero.policy = TRUE)
print(moran_global_TX)

# 9. Moran scatterplot
moran.plot(merged_proj$STROKE_AdjPrev, lw, zero.policy = TRUE)

# 10. Local Moran's I
local_moran <- localmoran(merged_proj$STROKE_AdjPrev, lw, zero.policy = TRUE)
merged_proj$local_I <- local_moran[, 1]
merged_proj$p_value <- local_moran[, 5]

local_moran_CA <- localmoran(merged_proj_CA$STROKE_AdjPrev, lw_CA, zero.policy = TRUE)
merged_proj_CA$local_I <- local_moran_CA[, 1]
merged_proj_CA$p_value <- local_moran_CA[, 5]

local_moran_GA <- localmoran(merged_proj_GA$STROKE_AdjPrev, lw_GA, zero.policy = TRUE)
merged_proj_GA$local_I <- local_moran_GA[, 1]
merged_proj_GA$p_value <- local_moran_GA[, 5]

local_moran_TX <- localmoran(merged_proj_TX$STROKE_AdjPrev, lw_TX, zero.policy = TRUE)
...
formula.0.glm <- STROKE_AdjPrev ~ 1 
formula.1.glm <- STROKE_AdjPrev ~ 1 + scale(cle8_point) 

## CA Models
formula.0.icar.CA <- STROKE_AdjPrev ~ 1 + f(county_int, model = "besag", graph = g_CA)
formula.1.icar.CA <- STROKE_AdjPrev ~ 1 + scale(cle8_point) + f(county_int, model = "besag", graph = g_CA)
formula.0.bym.CA <- STROKE_AdjPrev ~ 1 + f(county_int, model = "bym2", graph = g_CA)
formula.1.bym.CA <- STROKE_AdjPrev ~ 1 + scale(cle8_point) + f(county_int, model = "bym2", graph = g_CA)

## GA Models
formula.0.icar.GA <- STROKE_AdjPrev ~ 1 + f(county_int, model = "besag", graph = g_GA)
formula.1.icar.GA <- STROKE_AdjPrev ~ 1 + scale(cle8_point) + f(county_int, model = "besag", graph = g_GA)
formula.0.bym.GA <- STROKE_AdjPrev ~ 1 + f(county_int, model = "bym2", graph = g_GA)
formula.1.bym.GA <- STROKE_AdjPrev ~ 1 + scale(cle8_point) + f(county_int, model = "bym2", graph = g_GA)

## TX Models
formula.0.icar.TX <- STROKE_AdjPrev ~ 1 + f(county_int, model = "besag", graph = g_TX)
formula.1.icar.TX <- STROKE_AdjPrev ~ 1 + scale(cle8_point) + f(county_int, model = "besag", graph = g_TX)
formula.0.bym.TX <- STROKE_AdjPrev ~ 1 + f(county_int, model = "bym2", graph = g_TX)
formula.1.bym.TX <- STROKE_AdjPrev ~ 1 + scale(cle8_point) + f(county_int, model = "bym2", graph = g_TX)

##########   CA   ##########    
##### GLMs for All-cause mortality in CA
### Model 0: Intercept-only model
mod.0.CA.glm <- inla(formula.0.glm, family = "gaussian", data = merged_proj_CA, E = ExpectedSTROKE_AdjPrev,
...
mod.1.CA.glm <- inla(formula.1.glm, family = "gaussian", data = merged_proj_CA, E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 0: Intercept-only model
mod.0.CA.icar <- inla(formula.0.icar.CA, family = "gaussian", data = merged_proj_CA, E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate
mod.1.CA.icar <- inla(formula.1.icar.CA, family = "gaussian", data = merged_proj_CA, E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 0: Intercept-only model
mod.0.CA.bym <- inla(formula.0.bym.CA, family = "gaussian", data = merged_proj_CA, E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate
mod.1.CA.bym <- inla(formula.1.bym.CA, family = "gaussian", data = merged_proj_CA, E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 0: Intercept-only model
mod.0.GA.glm <- inla(formula.0.glm, family = "gaussian", data = merged_proj_GA, E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate
mod.1.GA.glm <- inla(formula.1.glm, family = "gaussian", data = merged_proj_GA, E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 0: Intercept-only model
mod.0.GA.icar <- inla(formula.0.icar.GA, family = "gaussian", data = merged_proj_GA, E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate
mod.1.GA.icar <- inla(formula.1.icar.GA, family = "gaussian", data = merged_proj_GA, E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 0: Intercept-only model
mod.0.GA.bym <- inla(formula.0.bym.GA, family = "gaussian", data = merged_proj_GA, E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate
mod.1.GA.bym <- inla(formula.1.bym.GA, family = "gaussian", data = merged_proj_GA, E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 0: Intercept-only model
mod.0.TX.glm <- inla(formula.0.glm, family = "gaussian", data = merged_proj_TX , E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate
mod.1.TX.glm <- inla(formula.1.glm, family = "gaussian", data = merged_proj_TX, E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 0: Intercept-only model
mod.0.TX.icar <- inla(formula.0.icar.TX, family = "gaussian", data = merged_proj_TX, E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate
mod.1.TX.icar <- inla(formula.1.icar.TX, family = "gaussian", data = merged_proj_TX, E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 0: Intercept-only model
mod.0.TX.bym <- inla(formula.0.bym.TX, family = "gaussian", data = merged_proj_TX, E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate
mod.1.TX.bym <- inla(formula.1.bym.TX, family = "gaussian", data = merged_proj_TX, E = ExpectedSTROKE_AdjPrev,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
write.csv(TX.var.res, "../Output/Results/STROKE_Morbidity_variance_components.csv", row.names = TRUE)
...
mod.0.CA.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering())
# ggsave("../Output/Figures/CA_BYM_Stroke_0.jpeg", plot = mod.0.CA.map, width = 4, height = 5, units = "in", dpi = 300)
...
mod.0.TX.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering()) 
# ggsave("../Output/Figures/TX_BYM_Stroke_0.jpeg", plot = mod.0.TX.map, width = 4, height = 5, units = "in", dpi = 300)
...
mod.0.GA.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering()) 
# ggsave("../Output/Figures/GA_BYM_Stroke_0.jpeg", plot = mod.0.GA.map, width = 4, height = 5, units = "in", dpi = 300)

combined.0 <- grid.arrange(mod.0.CA.map, mod.0.TX.map, mod.0.GA.map,  ncol = 2) 
ggsave("../Output/Figures/Combined_BYM_Stroke_0.jpeg", plot = combined.0, width = 6.67, height = 5, units = "in", dpi = 300)
...
mod.1.CA.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering())
# ggsave("../Output/Figures/CA_BYM_Stroke_1.jpeg", plot = mod.1.CA.map, width = 4, height = 5, units = "in", dpi = 300)
...
mod.1.TX.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering()) 
# ggsave("../Output/Figures/TX_BYM_Stroke_1.jpeg", plot = mod.1.TX.map, width = 4, height = 5, units = "in", dpi = 300)
...
mod.1.GA.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering()) 
# ggsave("../Output/Figures/GA_BYM_Stroke_1.jpeg", plot = mod.1.GA.map, width = 4, height = 5, units = "in", dpi = 300)

combined.1<-grid.arrange(mod.1.CA.map, mod.1.TX.map, mod.1.GA.map,  ncol = 2) 
ggsave("../Output/Figures/Combined_BYM_Stroke_1.jpeg", plot = combined.1, width = 6.67, height = 5, units = "in", dpi = 300)
...
mod.1.CA.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering())
# ggsave("../Output/Figures/CA_BYM_Stroke_Change.jpeg", plot = mod.1.CA.map, width = 4, height = 5, units = "in", dpi = 300)
...
mod.1.TX.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering()) 
# ggsave("../Output/Figures/TX_BYM_Stroke_Change.jpeg", plot = mod.1.TX.map, width = 4, height = 5, units = "in", dpi = 300)
...
mod.1.GA.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering()) 
# ggsave("../Output/Figures/GA_BYM_Stroke_Change.jpeg", plot = mod.1.GA.map, width = 4, height = 5, units = "in", dpi = 300)
...
# formula.0.exc.cvh <- STROKE_AdjPrev ~ 1 +
#   f(county_int, model = "iid", graph = g_CA)
# formula.1.exc.cvh <- STROKE_AdjPrev ~ 1 + scale(cle8_point) +
#   f(county_int, model = "iid", graph = g_CA)
# formula.2.exc.cvh <- STROKE_AdjPrev ~ 1 + scale(cle8_point) +
...
# formula.0.besag.cvh <- STROKE_AdjPrev ~ 1 +
#   f(county_int, model = "besag", graph = g_CA)
# formula.1.besag.cvh <- STROKE_AdjPrev ~ 1 + scale(cle8_point) +
#   f(county_int, model = "besag", graph = g_CA)
# formula.2.besag.cvh <- STROKE_AdjPrev ~ 1 + scale(cle8_point) +
...
# formula.0.bym.cvh <- STROKE_AdjPrev ~ 1 +
#   f(county_int, model = "bym2", graph = g_CA, scale.model = TRUE)
# formula.1.bym.cvh <- STROKE_AdjPrev ~ 1 + scale(cle8_point) +
#   f(county_int, model = "bym2", graph = g_CA, scale.model = TRUE)
# formula.2.bym.cvh <- STROKE_AdjPrev ~ 1 + scale(cle8_point) +
#   f(county_int, model = "bym2", graph = g_CA, scale.model = TRUE) +
