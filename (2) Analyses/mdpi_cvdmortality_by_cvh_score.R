...
library(tidyverse); library(sf); library(tigris); library(spdep);
library(ggplot2); library(ggspatial); library(readxl);
library(RColorBrewer); library(classInt); require(INLA); library(gridExtra);
library(viridis); library(arsenal); library(knitr); library(kableExtra);
library(ggpubr); library(rmarkdown); library(dplyr);
...
outcome.dat.cvd <- read.csv("../Data/cvd_mortality_2021-2023.csv") %>%
  dplyr::select(County.Code, Crude.Rate, Deaths, Population) %>%
...
outcome.dat.morbidity <- read.csv("../Data/PLACES__County_Data_(GIS_Friendly_Format),_2024_release_20260622.csv") %>%
  dplyr::select(CHD_AdjPrev, CHD_CrudePrev, STROKE_CrudePrev, STROKE_AdjPrev, CountyFIPS)


names(outcome.dat)
data <- data.1 %>%
  left_join(outcome.dat.cvd, by = c("CountyFIPS" = "County.Code")) %>%
  filter(!is.na(CVD_mortality)) 
data_CA <- data %>% filter(StateAbbr == "CA") %>%
            mutate(county_int = as.numeric(factor(CountyFIPS)),
                    county_slope = county_int, 
                    ExpectedCVDDeaths = sum(Deaths)/sum(Population)*Population)
...
descriptive_stats <- data %>%
  ...
  left_join(outcome.dat.morbidity, by = c("CountyFIPS")) %>%
  tableby(StateAbbr ~ cle8_point + CVD_mortality + AllCause_mortality + CHD_AdjPrev + STROKE_AdjPrev, data = .,
  control = tableby.control(test = FALSE, total = TRUE, numeric.stats = c("meansd", "range"), digits = 2))
summary(descriptive_stats)
# write2word(summary(descriptive_stats), file = "../Output/Results/Descriptive_Stats_CVH_CVD.doc")
# write2word(summary(descriptive_stats), file = "Descriptive_Stats_CVH_CVD.doc")
...
choropleth <- ggplot() +
  geom_sf(data = merged_sf, aes(fill = CVD_mortality), color = "white", size = 0.1) +
...
nb_TX <- poly2nb(merged_proj_CA)
nb_GA <- poly2nb(merged_proj_GA)
nb_TX <- poly2nb(merged_proj_TX)
lw_TX <- nb2listw(nb_CA, style = "W", zero.policy = TRUE)
lw_GA <- nb2listw(nb_GA, style = "W", zero.policy = TRUE)
lw_TX <- nb2listw(nb_TX, style = "W", zero.policy = TRUE)

# 8. Global Moran's I
moran_global <- moran.test(merged_proj$CVD_mortality, lw, zero.policy = TRUE)
print(moran_global)

moran_global_CA <- moran.test(merged_proj_CA$CVD_mortality, lw_CA, zero.policy = TRUE)
print(moran_global_CA)
moran_global_GA <- moran.test(merged_proj_GA$CVD_mortality, lw_GA, zero.policy = TRUE)
print(moran_global_GA)
moran_global_TX <- moran.test(merged_proj_TX$CVD_mortality, lw_TX, zero.policy = TRUE)
print(moran_global_TX)

# 9. Moran scatterplot
moran.plot(merged_proj$CVD_mortality, lw, zero.policy = TRUE)

# 10. Local Moran's I
local_moran <- localmoran(merged_proj$CVD_mortality, lw, zero.policy = TRUE)
merged_proj$local_I <- local_moran[, 1]
merged_proj$p_value <- local_moran[, 5]

local_moran_GA <- localmoran(merged_proj_GA$CVD_mortality, lw_GA, zero.policy = TRUE)
merged_proj_GA$local_I <- local_moran_GA[, 1]
merged_proj_GA$p_value <- local_moran_GA[, 5]

local_moran_CA <- localmoran(merged_proj_CA$CVD_mortality, lw_CA, zero.policy = TRUE)
merged_proj_CA$local_I <- local_moran_CA[, 1]
merged_proj_CA$p_value <- local_moran_CA[, 5]

local_moran_TX <- localmoran(merged_proj_TX$CVD_mortality, lw_TX, zero.policy = TRUE)
...
formula.0.glm.cvh <- Deaths ~ 1 
formula.1.glm.cvh <- Deaths ~ 1 + scale(cle8_point) 
...
##########   CA   ##########    
##### GLMs for CVD mortality in CA
### Model 0: Intercept-only model
mod.0.CA.glm <- inla(formula.0.glm.cvh, family = "nbinomial", data = merged_proj_CA, E = ExpectedCVDDeaths,
...
mod.1.CA.glm <- inla(formula.1.glm.cvh, family = "nbinomial", data = merged_proj_CA, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
##### GLMMs with ICAR for CVD mortality in CA
### Model 0: Intercept-only model
mod.0.CA.icar <- inla(formula.0.icar.CA, family = "nbinomial", data = merged_proj_CA, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate
mod.1.CA.icar <- inla(formula.1.icar.CA, family = "nbinomial", data = merged_proj_CA, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
##### GLMMs with Besag-York-Mollie for CVD mortality in CA
### Model 0: Intercept-only model
mod.0.CA.bym <- inla(formula.0.bym.CA, family = "nbinomial", data = merged_proj_CA, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate
mod.1.CA.bym <- inla(formula.1.bym.CA, family = "nbinomial", data = merged_proj_CA, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
##########   GA   ##########    
##### GLMs for CVD mortality in GA
### Model 0: Intercept-only model
mod.0.GA.glm <- inla(formula.0.glm.cvh, family = "nbinomial", data = merged_proj_GA, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate
mod.1.GA.glm <- inla(formula.1.glm.cvh, family = "nbinomial", data = merged_proj_GA, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
##### GLMMs with ICAR for CVD mortality in GA
### Model 0: Intercept-only model
mod.0.GA.icar <- inla(formula.0.icar.GA, family = "nbinomial", data = merged_proj_GA, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate
mod.1.GA.icar <- inla(formula.1.icar.GA, family = "nbinomial", data = merged_proj_GA, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
##### GLMMs with Besag-York-Mollie for CVD mortality in GA
### Model 0: Intercept-only model
mod.0.GA.bym <- inla(formula.0.bym.GA, family = "nbinomial", data = merged_proj_GA, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate
mod.1.GA.bym <- inla(formula.1.bym.GA, family = "nbinomial", data = merged_proj_GA, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
GA.fixed = do.call(rbind, lapply(GA.mod.results, function(x){extract.fixed.effects(x)}))
##########   TX   ##########    
##### GLMs for CVD mortality in TX
### Model 0: Intercept-only model
mod.0.TX.glm <- inla(formula.0.glm.cvh, family = "nbinomial", data = merged_proj_TX, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate
mod.1.TX.glm <- inla(formula.1.glm.cvh, family = "nbinomial", data = merged_proj_TX, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
##### GLMMs with ICAR for CVD mortality in TX
### Model 0: Intercept-only model
mod.0.TX.icar <- inla(formula.0.icar.TX, family = "nbinomial", data = merged_proj_TX, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate
mod.1.TX.icar <- inla(formula.1.icar.TX, family = "nbinomial", data = merged_proj_TX, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
##### GLMMs with Besag-York-Mollie for CVD mortality in TX
### Model 0: Intercept-only model
mod.0.TX.bym <- inla(formula.0.bym.TX, family = "nbinomial", data = merged_proj_TX, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate
mod.1.TX.bym <- inla(formula.1.bym.TX, family = "nbinomial", data = merged_proj_TX, E = ExpectedCVDDeaths,
                  control.predictor = list(compute = TRUE),
                  control.compute = list(return.marginals.predictor = TRUE, 
                                         dic = TRUE, cpo = TRUE, waic = TRUE))
...
### Model 1: CVH score as a covariate - with outliers removed
mod.1.a.TX.bym <- merged_proj_TX %>%
      filter(CVD_mortality < 650) %>%
      inla(formula.1.bym.TX, family = "nbinomial", data = ., E = ExpectedCVDDeaths,
            control.predictor = list(compute = TRUE),
            control.compute = list(return.marginals.predictor = TRUE, 
            dic = TRUE, cpo = TRUE, waic = TRUE))
summary(mod.1.a.TX.bym)
sum(mod.1.a.TX.bym$cpo$cpo) 
hist(mod.1.a.TX.bym$cpo$pit) # data do not fit well (https://faculty.washington.edu/jonno/SISMIDmaterial/3-spatial1.pdf)

mod.1.b.TX <- merged_proj_TX %>%
      st_drop_geometry() %>%
      glm(Deaths ~ scale(cle8_point) , offset=log(Population), data = .,
      weights = ExpectedCVDDeaths, family = quasipoisson(link = "log")) 
summary(mod.1.b.TX)
...
write.csv(TX.var.res, "../Output/Results/CVD_Mortality_variance_components.csv", row.names = TRUE)
...
TX.out
TX.fixed = do.call(rbind, lapply(TX.mod.results, function(x){extract.fixed.effects(x)}))

merged_proj_TX %>% st_drop_geometry() %>%
    select(cle8_point, CVD_mortality) %>%
    plot()

hist(merged_proj_TX$CVD_mortality, breaks = 30)
hist(merged_proj_TX$cle8_point, breaks = 30)
    

merged_proj_TX %>% st_drop_geometry() %>%
    select(cle8_point, CVD_mortality, CountyName, Population) %>%
    arrange(by = CVD_mortality)

write.csv(rbind(CA.out %>% as.data.frame() %>% arrange(DIC),
                GA.out %>% as.data.frame() %>% arrange(DIC),
                TX.out %>% as.data.frame() %>% arrange(DIC)),
          "../Output/Results/CVD_Mortality_NB-model_fit_stats.csv",
          row.names = TRUE)

write.csv(rbind(CA.fixed, GA.fixed, TX.fixed),
          "../Output/Results/CVD_Mortality_NB-model_fixed_effects.csv",
          row.names = TRUE)
...
mod.0.CA.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering())
# ggsave("../Output/Figures/CA_BYM_CVDMort_0.jpeg", plot = mod.0.CA.map, width = 4, height = 5, units = "in", dpi = 300)
...
mod.0.TX.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering()) 
# ggsave("../Output/Figures/TX_BYM_CVDMort_0.jpeg", plot = mod.0.TX.map, width = 4, height = 5, units = "in", dpi = 300)
...
mod.0.GA.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering()) 
# ggsave("../Output/Figures/GA_BYM_CVDMort_0.jpeg", plot = mod.0.GA.map, width = 4, height = 5, units = "in", dpi = 300)

combined.0 <- grid.arrange(mod.0.CA.map, mod.0.TX.map, mod.0.GA.map,  ncol = 2) 
ggsave("../Output/Figures/Combined_BYM_CVDMort_0.jpeg", plot = combined.0, width = 6.67, height = 5, units = "in", dpi = 300)
...
mod.1.CA.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering())
ggsave("../Output/Figures/CA_BYM_CVDMort_1.jpeg", plot = mod.1.CA.map, width = 4, height = 5, units = "in", dpi = 300)
...
mod.1.TX.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering()) 
ggsave("../Output/Figures/TX_BYM_CVDMort_1.jpeg", plot = mod.1.TX.map, width = 4, height = 5, units = "in", dpi = 300)
...
mod.1.GA.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering()) 
ggsave("../Output/Figures/GA_BYM_CVDMort_1.jpeg", plot = mod.1.GA.map, width = 4, height = 5, units = "in", dpi = 300)

combined.1<-grid.arrange(mod.1.CA.map, mod.1.TX.map, mod.1.GA.map,  ncol = 2) 
ggsave("../Output/Figures/Combined_BYM_CVDMort_1.jpeg", plot = combined.1, width = 6.67, height = 5, units = "in", dpi = 300)
...
mod.1.CA.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering())
# ggsave("../Output/Figures/CA_BYM_CVDMort_Change.jpeg", plot = mod.1.CA.map, width = 4, height = 5, units = "in", dpi = 300)
...
mod.1.TX.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering()) 
# ggsave("../Output/Figures/TX_BYM_CVDMort_Change.jpeg", plot = mod.1.TX.map, width = 4, height = 5, units = "in", dpi = 300)
...
mod.1.GA.map <- ggplot() +
 ...
 #  geom_sf(data = states_sf, fill = NA, color = "grey30", size = 0.3) +
  ...
  # annotation_north_arrow(location = "bl", which_north = "true",
  #                        style = north_arrow_fancy_orienteering()) 
# ggsave("../Output/Figures/GA_BYM_CVDMort_Change.jpeg", plot = mod.1.GA.map, width = 4, height = 5, units = "in", dpi = 300)
...
ggsave("../Output/Figures/Combined_RE_CVDMort_Change_Maps.jpeg", plot = combined.1, width = 6.67, height = 5, units = "in", dpi = 300)

##################      ARCHIVED MATERIAL      ################## 
# ### Model 2: Exchangeable random effect model
# mod.2.CA.exc <- inla(formula.2.exc.cvh, family = "nbinomial", data = merged_proj_CA,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
...
# sum(mod.2.CA.exc$cpo$cpo) 
# hist(mod.2.CA.exc$cpo$pit) # data do not fit well (https://faculty.washington.edu/jonno/SISMIDmaterial/3-spatial1.pdf)

# ##### ICAR random effect model for CVD mortality in CA
# formula.0.besag.cvh <- CVD_mortality ~ 1 +
#   f(county_int, model = "besag", graph = g_CA)
# formula.1.besag.cvh <- CVD_mortality ~ 1 + scale(cle8_point) +
#   f(county_int, model = "besag", graph = g_CA)
# formula.2.besag.cvh <- CVD_mortality ~ 1 + scale(cle8_point) +
#   f(county_int, model = "besag", graph = g_CA) + f(county_slope, scale(cle8_point), model = "iid", graph = g_CA)

# ### Model 0: Besag-ICAR
# mod.0.CA.besag <- inla(formula.0.besag.cvh, family = "nbinomial", data = merged_proj_CA,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
...
# ### Model 1: Besag-ICAR
# mod.1.CA.besag <- inla(formula.1.besag.cvh, family = "nbinomial", data = merged_proj_CA,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
...
# ### Model 2: Besag-ICAR
# mod.2.CA.besag <- inla(formula.2.besag.cvh, family = "nbinomial", data = merged_proj_CA,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
...
# ##### Besag-York-Mollie random effect model for CVD mortality in CA
# formula.0.bym.cvh <- CVD_mortality ~ 1 +
#   f(county_int, model = "bym2", graph = g_CA, scale.model = TRUE)
# formula.1.bym.cvh <- CVD_mortality ~ 1 + scale(cle8_point) +
#   f(county_int, model = "bym2", graph = g_CA, scale.model = TRUE)
# formula.2.bym.cvh <- CVD_mortality ~ 1 + scale(cle8_point) +
#   f(county_int, model = "bym2", graph = g_CA, scale.model = TRUE) +
#   f(county_slope, scale(cle8_point), model = "iid", graph = g_CA)

# ### Model 0: BYM2
# mod.0.CA.bym <- inla(formula.0.bym.cvh, family = "nbinomial", data = merged_proj_CA,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
# summary(mod.0.CA.bym)
...
# ### Model 1: BYM2
# mod.1.CA.bym <- inla(formula.1.bym.cvh, family = "nbinomial", data = merged_proj_CA,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
# summary(mod.1.CA.bym)
...
# ### Model 2: BYM2
# mod.2.CA.bym <- inla(formula.2.bym.cvh, family = "nbinomial", data = merged_proj_CA,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
...
# colnames(CA.out) = c("DIC", "pD", "WAIC", "pWAIC", "Marginal Likelihood", "Log Marginal Likelihood")
...
# ##########   TX   ##########    
# formula.0.exc.cvh <- CVD_mortality ~ 1 +
#   f(county_int, model = "iid", graph = g_TX)
# formula.1.exc.cvh <- CVD_mortality ~ 1 + scale(cle8_point) +
#   f(county_int, model = "iid", graph = g_TX)
# formula.2.exc.cvh <- CVD_mortality ~ 1 + scale(cle8_point) +
#   f(county_int, model = "iid", graph = g_TX) + f(county_slope, scale(cle8_point), model = "iid", graph = g_TX)

# ##### Exchangeable random effect model for CVD mortality in TX
# ### Model 0: Exchangeable random effect model
# mod.0.TX.exc <- inla(formula.0.exc.cvh, family = "nbinomial", data = merged_proj_TX,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
...
# mod.1.TX.exc <- inla(formula.1.exc.cvh, family = "nbinomial", data = merged_proj_TX,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
...
# ### Model 0: Besag-ICAR
# mod.0.TX.besag <- inla(formula.0.besag.cvh, family = "nbinomial", data = merged_proj_TX,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
...
# ### Model 1: Besag-ICAR
# mod.1.TX.besag <- inla(formula.1.besag.cvh, family = "nbinomial", data = merged_proj_TX,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
...
# ### Model 2: Besag-ICAR
# mod.2.TX.besag <- inla(formula.2.besag.cvh, family = "nbinomial", data = merged_proj_TX,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
...
# ##### Mesag-York_Mollie random effect model for CVD mortality in TX
# formula.0.bym.cvh <- CVD_mortality ~ 1 +
...
# ### Model 0: BYM2
# mod.0.TX.bym <- inla(formula.0.bym.cvh, family = "nbinomial", data = merged_proj_TX,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
# summary(mod.0.TX.bym)
...
# ### Model 1: BYM2
# mod.1.TX.bym <- inla(formula.1.bym.cvh, family = "nbinomial", data = merged_proj_TX,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
# summary(mod.1.TX.bym)
...
# ### Model 2: BYM2
# mod.2.TX.bym <- inla(formula.2.bym.cvh, family = "nbinomial", data = merged_proj_TX,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
...
# TX.mod.results = list(mod.0.TX.exc, mod.1.TX.exc, mod.2.TX.exc,
#                      mod.0.TX.besag, mod.1.TX.besag, mod.2.TX.besag,
#                      mod.0.TX.bym, mod.1.TX.bym, mod.2.TX.bym)
...
# rownames(TX.out) = c("Model 0: Exchangeable", "Model 1: Exchangeable", "Model 2: Exchangeable",
#                      "Model 0: Besag-ICAR", "Model 1: Besag-ICAR", "Model 2: Besag-ICAR",
#                      "Model 0: BYM2", "Model 1: BYM2", "Model 2: BYM2")
...
# ##### Exchangeable random effect model for CVD mortality in GA
# ### Model 0: Exchangeable random effect model
# mod.0.GA.exc <- inla(formula.0.exc.cvh, family = "nbinomial", data = merged_proj_GA,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
...
# ### Model 1: Exchangeable random effect model
# mod.1.GA.exc <- inla(formula.1.exc.cvh, family = "nbinomial", data = merged_proj_GA,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
...
# ### Model 0: Besag-ICAR
# mod.0.GA.besag <- inla(formula.0.besag.cvh, family = "nbinomial", data = merged_proj_GA,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
...
# ### Model 1: Besag-ICAR
# mod.1.GA.besag <- inla(formula.1.besag.cvh, family = "nbinomial", data = merged_proj_GA,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
...
# ### Model 2: Besag-ICAR
# mod.2.GA.besag <- inla(formula.2.besag.cvh, family = "nbinomial", data = merged_proj_GA,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
...
# ##### Mesag-York_Mollie random effect model for CVD mortality in GA
# formula.0.bym.cvh <- CVD_mortality ~ 1 +
...
# ### Model 0: BYM2
# mod.0.GA.bym <- inla(formula.0.bym.cvh, family = "nbinomial", data = merged_proj_GA,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
# summary(mod.0.GA.bym)
...
# ### Model 1: BYM2
# mod.1.GA.bym <- inla(formula.1.bym.cvh, family = "nbinomial", data = merged_proj_GA,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
# summary(mod.1.GA.bym)
...
# ### Model 2: BYM2
# mod.2.GA.bym <- inla(formula.2.bym.cvh, family = "nbinomial", data = merged_proj_GA,
#                   control.predictor = list(compute = TRUE),
#                   control.compute = list(return.marginals.predictor = TRUE, 
#                                          dic = TRUE, cpo = TRUE, waic = TRUE))
