...
choropleth <- ggplot() +
  geom_sf(data = merged_sf, aes(fill = CHD_AdjPrev), color = "white", size = 0.1) +
...
# 8. Global Moran's I
moran_global <- moran.test(merged_proj$CHD_AdjPrev, lw, zero.policy = TRUE)
print(moran_global)

moran_global_CA <- moran.test(merged_proj_CA$CHD_AdjPrev, lw_CA, zero.policy = TRUE)
print(moran_global_CA)
moran_global_GA <- moran.test(merged_proj_GA$CHD_AdjPrev, lw_GA, zero.policy = TRUE)
print(moran_global_GA)
moran_global_TX <- moran.test(merged_proj_TX$CHD_AdjPrev, lw_TX, zero.policy = TRUE)
print(moran_global_TX)

# 9. Moran scatterplot
moran.plot(merged_proj$CHD_AdjPrev, lw, zero.policy = TRUE)

# 10. Local Moran's I
local_moran <- localmoran(merged_proj$CHD_AdjPrev, lw, zero.policy = TRUE)
merged_proj$local_I <- local_moran[, 1]
merged_proj$p_value <- local_moran[, 5]

local_moran_CA <- localmoran(merged_proj_CA$CHD_AdjPrev, lw_CA, zero.policy = TRUE)
merged_proj_CA$local_I <- local_moran_CA[, 1]
merged_proj_CA$p_value <- local_moran_CA[, 5]

local_moran_GA <- localmoran(merged_proj_GA$CHD_AdjPrev, lw_GA, zero.policy = TRUE)
merged_proj_GA$local_I <- local_moran_GA[, 1]
merged_proj_GA$p_value <- local_moran_GA[, 5]

local_moran_TX <- localmoran(merged_proj_TX$CHD_AdjPrev, lw_TX, zero.policy = TRUE)
...
formula.0.glm <- CHD_AdjPrev ~ 1 
formula.1.glm <- CHD_AdjPrev ~ 1 + scale(cle8_point) 

## CA Models
formula.0.icar.CA <- CHD_AdjPrev ~ 1 + f(county_int, model = "besag", graph = g_CA)
formula.1.icar.CA <- CHD_AdjPrev ~ 1 + scale(cle8_point) + f(county_int, model = "besag", graph = g_CA)
formula.0.bym.CA <- CHD_AdjPrev ~ 1 + f(county_int, model = "bym2", graph = g_CA)
formula.1.bym.CA <- CHD_AdjPrev ~ 1 + scale(cle8_point) + f(county_int, model = "bym2", graph = g_CA)

## GA Models
formula.0.icar.GA <- CHD_AdjPrev ~ 1 + f(county_int, model = "besag", graph = g_GA)
formula.1.icar.GA <- CHD_AdjPrev ~ 1 + scale(cle8_point) + f(county_int, model = "besag", graph = g_GA)
formula.0.bym.GA <- CHD_AdjPrev ~ 1 + f(county_int, model = "bym2", graph = g_GA)
formula.1.bym.GA <- CHD_AdjPrev ~ 1 + scale(cle8_point) + f(county_int, model = "bym2", graph = g_GA)

## TX Models
formula.0.icar.TX <- CHD_AdjPrev ~ 1 + f(county_int, model = "besag", graph = g_TX)
formula.1.icar.TX <- CHD_AdjPrev ~ 1 + scale(cle8_point) + f(county_int, model = "besag", graph = g_TX)
formula.0.bym.TX <- CHD_AdjPrev ~ 1 + f(county_int, model = "bym2", graph = g_TX)
...
# formula.0.exc.cvh <- CHD_AdjPrev ~ 1 +
#   f(county_int, model = "iid", graph = g_CA)
# formula.1.exc.cvh <- CHD_AdjPrev ~ 1 + scale(cle8_point) +
#   f(county_int, model = "iid", graph = g_CA)
# formula.2.exc.cvh <- CHD_AdjPrev ~ 1 + scale(cle8_point) +
...
# formula.0.besag.cvh <- CHD_AdjPrev ~ 1 +
#   f(county_int, model = "besag", graph = g_CA)
# formula.1.besag.cvh <- CHD_AdjPrev ~ 1 + scale(cle8_point) +
#   f(county_int, model = "besag", graph = g_CA)
# formula.2.besag.cvh <- CHD_AdjPrev ~ 1 + scale(cle8_point) +
...
# formula.0.bym.cvh <- CHD_AdjPrev ~ 1 +
#   f(county_int, model = "bym2", graph = g_CA, scale.model = TRUE)
# formula.1.bym.cvh <- CHD_AdjPrev ~ 1 + scale(cle8_point) +
#   f(county_int, model = "bym2", graph = g_CA, scale.model = TRUE)
# formula.2.bym.cvh <- CHD_AdjPrev ~ 1 + scale(cle8_point) +
#   f(county_int, model = "bym2", graph = g_CA, scale.model = TRUE) +
